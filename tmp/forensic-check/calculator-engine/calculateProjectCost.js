...
const roomCountAddons_1 = require("./modules/roomCountAddons");
...
function calculateProjectCost(input) {
    ...
    const benchmarkEffectiveArea = Math.max(0, buildingArea)
        + basementBaseCosts.breakdownItems.reduce((sum, item) => (sum + (Math.max(0, item.area) * item.benchmarkRateFactor)), 0)
    ...
    const roomCountAddons = (0, roomCountAddons_1.calculateRoomCountAddons)({
        qualityId,
        bathroomDelta,
        wcDelta,
    });
    const kg300SubgroupCosts = Object.assign(Object.assign({}, kg300ModifierResult.kg300SubgroupCosts), { subgroup340Cost: kg300ModifierResult.kg300SubgroupCosts.subgroup340Cost
            + roomCountAddons.kg300SubgroupAddons.subgroup340Cost, subgroup350Cost: kg300ModifierResult.kg300SubgroupCosts.subgroup350Cost
    ...
    const mainKg400CategoryCostsById = addCostsById(kg400Costs.categoryCostsById, roomCountAddons.kg400CategoryAddonsById);
    const mergedKg400CategoryCostsById = addCostsById(mainKg400CategoryCostsById, basementBaseCosts.basementKg400CategoryCostsById);
