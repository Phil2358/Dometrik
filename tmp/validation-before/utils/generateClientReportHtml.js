...
exports.generateClientReportHtml = generateClientReportHtml;
...
function getSizeCorrectionText(factor) {
    const label = (0, construction_1.formatSizeCorrectionFactorLabel)(factor);
...
function buildAssumptions(data) {
    ...
    if (data.basementArea > 0) {
        assumptions.push(`Basement mix: ${(0, computeScenarioCosts_1.formatBasementSummary)(data.storageBasementArea, data.parkingBasementArea, data.habitableBasementArea)}`);
        assumptions.push(`Basement DIN contribution: ${(0, construction_1.formatEuro)(data.basementBaseCost)} integrated into KG 300 and KG 400`);
    }
    if (data.includePool) {
...
function buildChartSegments(data) {
    const items = [
    ...
    return items
        .filter((i) => i.value > 0)
        .map((i) => (Object.assign(Object.assign({}, i), { color: CHART_COLORS[colorIdx++ % CHART_COLORS.length], percent: Math.round((i.value / total) * 100) })));
}
...
function generateClientReportHtml(data, reportTitle) {
    ...
    if (data.terraceArea > 0) {
        overviewItems.push({ label: 'Covered Terraces', value: `${data.terraceArea} m² (50% factor) · ${(0, construction_1.formatEuro)(data.coveredTerracesBenchmarkContribution)} benchmark contribution` });
    }
    if (data.balconyArea > 0) {
        overviewItems.push({ label: 'Balcony Area', value: `${data.balconyArea} m² (30% factor) · ${(0, construction_1.formatEuro)(data.balconyAreaBenchmarkContribution)} benchmark contribution` });
    }
    overviewItems.push({ label: 'Building Area benchmark', value: (0, construction_1.formatEuro)(data.baseBuildingAreaBenchmarkContribution) });
    overviewItems.push({ label: 'Total benchmark before allocation', value: (0, construction_1.formatEuro)(data.totalBenchmarkContributionBeforeGroupAllocation) });
    if (data.basementArea > 0) {
        overviewItems.push({ label: 'Basement', value: (0, computeScenarioCosts_1.formatBasementSummary)(data.storageBasementArea, data.parkingBasementArea, data.habitableBasementArea) });
        overviewItems.push({ label: 'Basement contribution', value: `${(0, construction_1.formatEuro)(data.basementBaseCost)} integrated into DIN` });
        overviewItems.push({ label: 'Basement -> KG 300', value: (0, construction_1.formatEuro)(data.basementKg300Total) });
        overviewItems.push({ label: 'Basement -> KG 400', value: (0, construction_1.formatEuro)(data.basementKg400Total) });
    }
    if (data.includePool) {
    ...
    const breakdownHtml = breakdownRows
      ...
      <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid ${BORDER_LIGHT};">
        ...
        <span style="font-size:10.5px;font-weight:600;color:${PRIMARY};">${(0, construction_1.formatEuro)(r.value)}</span>
      </div>`)
        .join('');
    const assumptionsHtml = assumptions
        .map((a) => `<div style="font-size:10px;color:${TEXT_BODY};padding:3px 0 3px 14px;position:relative;line-height:1.6;">
...
<div class="no-break" style="margin-bottom:4px;">
  <div style="background:linear-gradient(135deg, ${PRIMARY} 0%, ${PRIMARY_LIGHT} 100%);border-radius:10px;padding:26px 20px;text-align:center;">
    ...
    <div style="font-size:38px;font-weight:800;color:#FFFFFF;letter-spacing:-1.2px;line-height:1.1;">${(0, construction_1.formatEuro)(data.totalCost)}</div>
  </div>
</div>
...
<div class="no-break" style="margin-bottom:14px;">
  ...
  <div style="padding:0 2px;">
    ...
    <div style="display:flex;justify-content:space-between;padding:10px 0 4px 0;border-top:2px solid ${PRIMARY};margin-top:4px;">
      ...
      <span style="font-size:11.5px;font-weight:800;color:${PRIMARY};">${(0, construction_1.formatEuro)(data.totalCost)}</span>
    </div>
  </div>
</div>
...
<div class="page-two">
  ...
  <div class="page-two-section" style="margin-bottom:20px;">
    ...
    <div style="background:${BG_WARM};border:1px solid ${BORDER};border-radius:8px;padding:18px 22px;text-align:center;">
      ...
      <div style="font-size:22px;font-weight:800;color:${PRIMARY};letter-spacing:-0.5px;">${(0, construction_1.formatEuro)(data.estimatedRangeLow)}  –  ${(0, construction_1.formatEuro)(data.estimatedRangeHigh)}</div>
