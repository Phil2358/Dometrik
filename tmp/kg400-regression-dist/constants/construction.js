...
export function getResidentialProgramBaseline(livingArea) {
...
export function getKitchenAreaFactor(effectiveArea) {
    if (effectiveArea <= 80)
        return 0.85;
    if (effectiveArea <= 140)
        return 1.00;
    if (effectiveArea <= 220)
        return 1.20;
    if (effectiveArea <= 320)
        return 1.40;
    return 1.65;
}
export function getSuggestedGeneralFurnitureBaseAmount(effectiveArea, bedroomCount) {
    let areaBase = 4500;
    if (effectiveArea > 80)
        areaBase = 5500;
    if (effectiveArea > 140)
...
export const GROUNDWATER_CONDITIONS = [
    {
        id: 'normal',
...
export const SITE_ACCESSIBILITY_OPTIONS = [
    {
        id: 'normal',
...
export function getPlotSizeFactor(plotSize) {
    if (plotSize <= 800)
        return 0.90;
    if (plotSize <= 2000)
        return 1.00;
...
export function clampSitePreparationMultiplier(multiplier) {
    return Math.min(1.80, Math.max(0.80, multiplier));
}
export function getSizeCorrectionFactor(livingArea) {
    if (livingArea < 120)
        return 1.10;
    if (livingArea < 160)
        return 1.05;
...
export function getSizeCorrectionLabel(livingArea) {
    if (livingArea < 120)
        return '+10%';
    if (livingArea < 160)
        return '+5%';
...
export function getBasementExcavationCost(basementArea, siteCondition, groundwaterCondition) {
    if (basementArea <= 0)
        return 0;
...
export function getBasementStructureCost(basementArea, basementType, groundwaterCondition) {
    if (basementArea <= 0)
        return 0;
...
export function getBasementRockyAdjustment(basementArea) {
    if (basementArea <= 50)
        return 0;
    if (basementArea <= 120)
        return 0.05;
...
export function getLandscapingSizeAdjustment(area) {
    if (area <= 300)
        return 0;
    if (area <= 800)
        return 0.10;
...
export const POOL_QUALITY_OPTIONS = [
...
export function getPoolDepthFactor(depth) {
    if (depth <= 1.20)
        return 0.90;
    if (depth <= 1.50)
        return 1.00;
...
export const POOL_TERRAIN_MULTIPLIERS = {
    flat_normal: 1.00,
    flat_rocky: 1.10,
...
export function getUtilityConnectionGroupCosts(optionId, totalCost) {
    var _a;
...
export const DIN276_GROUPS = {
    KG100: { code: 'KG 100', name: (_b = (_a = getDin276Group('100')) === null || _a === void 0 ? void 0 : _a.label) !== null && _b !== void 0 ? _b : 'Land' },
    KG200: { code: 'KG 200', name: (_d = (_c = getDin276Group('200')) === null || _c === void 0 ? void 0 : _c.label) !== null && _d !== void 0 ? _d : 'Preparatory Measures' },
    KG300: { code: 'KG 300', name: (_f = (_e = getDin276Group('300')) === null || _e === void 0 ? void 0 : _e.label) !== null && _f !== void 0 ? _f : 'Building - Construction Works' },
    KG400: { code: 'KG 400', name: (_h = (_g = getDin276Group('400')) === null || _g === void 0 ? void 0 : _g.label) !== null && _h !== void 0 ? _h : 'Technical Systems' },
    KG500: { code: 'KG 500', name: (_k = (_j = getDin276Group('500')) === null || _j === void 0 ? void 0 : _j.label) !== null && _k !== void 0 ? _k : 'External Works and Open Spaces' },
    KG600: { code: 'KG 600', name: (_m = (_l = getDin276Group('600')) === null || _l === void 0 ? void 0 : _l.label) !== null && _m !== void 0 ? _m : 'Furnishings and Artworks' },
...
export const KG300_CATEGORY_IDS = ['concrete', 'masonry', 'roofing', 'insulation', 'windows', 'interior'];
export const KG400_CATEGORY_IDS = ['plumbing', 'heating', 'ventilation_cooling', 'electrical', 'data_security', 'automation'];
export const KG500_CATEGORY_IDS = [];
export const KG600_CATEGORY_IDS = ['furnishings'];
...
export const PERMIT_DESIGN_QUALITY_MULTIPLIERS = {
    standard: 1.00,
    premium: 1.15,
    luxury: 1.30,
};
