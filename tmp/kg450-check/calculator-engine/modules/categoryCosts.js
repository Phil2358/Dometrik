...
function calculateLevel1BenchmarkAllocation(input) {
    ...
    const benchmarkRemainderAfterFixed210 = Math.max(0, Math.round(input.benchmarkTotal) - fixedBenchmarkIncluded);
    const fixedWardrobeBaselineBenchmarkIncluded = Math.max(0, input.fixedWardrobeBaselineCost);
    const benchmarkRemainderAfterFixed210And620 = Math.max(0, benchmarkRemainderAfterFixed210 - fixedWardrobeBaselineBenchmarkIncluded);
    const kg450BaselineEssentialCost = Math.round(benchmarkRemainderAfterFixed210And620 * construction_1.KG450_BASELINE_ESSENTIAL_RATE);
    const remainingBenchmarkPool = Math.max(0, benchmarkRemainderAfterFixed210And620
    ...
    return {
        ...
        kg450BaselineEssentialRate: construction_1.KG450_BASELINE_ESSENTIAL_RATE,
        kg450BaselineEssentialCost,
